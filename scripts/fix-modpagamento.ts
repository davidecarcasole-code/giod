      ...
      for (let i = hdrIx + 1; i < rows.length; i++) {
        ...
        try {
          ...
          let mp = await prisma.modPagamento.findFirst({
            where: { name: modPagamentoName, sedeId: sede.id },
          });
          if (!mp) {
            mp = await prisma.modPagamento.findFirst({
              where: { name: modPagamentoName, sedeId: null },
            });
          }
          if (!mp) {
            mp = await prisma.modPagamento.create({
              data: { name: modPagamentoName, sedeId: sede.id },
            });
          }

          if (patient.modPagamentoId !== mp.id) {
            await prisma.patient.update({
              where: { id: patient.id },
              data: { modPagamentoId: mp.id },
            });
            updated++;
            fileUpdated++;
          }
